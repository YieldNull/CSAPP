 ...
 *
 *  Use implicit free list and next fit algorithm
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
static char *fitted_header; // header address of last fitted block
...
void *mm_malloc(size_t size) {
    ...
    char *header = fitted_header;
    unsigned int found = 0;

    // start at last fitted header
    do {
    ...
    } while (header != fitted_header);


    if (found == 0) { // use mem_sbrk
...
void mm_free(void *block) {
    ...
    SET_WORD_AT(new_header, real_size);
    SET_WORD_AT(new_footer, real_size);

    if (new_header <= fitted_header && fitted_header <= new_footer) {
        fitted_header = new_header;
    }
